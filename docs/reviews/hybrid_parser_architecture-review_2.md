...
Monitor memory on large saves (turn 400+) 2. Parallel Parsing Constraints
...
Document that parallel parsing will complete all tasks even if one fails 4. Serde Performance
...
Document that caching is optional 5. Two-Pass Insertion Complexity
...
Document which entities need two-pass (characters, cities with founders, etc.) 6. Testing Gap: Integration with Real Saves
...
// Feature flag approach: #[cfg(feature = "hybrid-parser")]
...
Consider: save GameData to temp file before insertion 2. Missing: Benchmarking Baseline
...
diff baseline.txt hybrid.txt 3. Missing: Breaking Changes
