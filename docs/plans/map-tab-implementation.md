...
**`src-tauri/src/lib.rs`** - Add command (~after other get_* commands):
...
<script lang="ts">
  ...
  type ColorMode = "political" | "terrain" | "height" | "vegetation" | "resource";
  ...
  import { getTerrainColor, getHeightColor, getVegetationColor } from "$lib/config";
