...
**Rule:** Each round, every still-active player in a division is paired against one opponent with a similar record; the system avoids rematches where it can. The pairing is **advisory** — an admin can edit it before the round starts.
...
4. **Rematch avoidance:** if a pairing repeats a prior match, try swapping one bottom-half player with another to break it — but only if the swap doesn't create a _new_ rematch. If no clean swap exists, the rematch is accepted (the admin can still override).
5. **Odd bucket:** the lowest-ranked player in the bucket "floats down" to the next bucket and is paired there.

> **vs. textbook Swiss:** This is close to standard Swiss (records buckets, fold pairing, rematch avoidance). The differences are the early-exit pool (clinched players sit out) and that pairing is advisory rather than locked.
...
- **Advisory:** like all pairings, the admin can reassign the bye before the round starts.
