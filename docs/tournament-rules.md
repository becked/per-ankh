...
- **Caster self-service:** any logged-in user can add/move/remove **themselves** on a pending match's part (`…/parts/:part_id/casters/me`, `player.ts`) — streamer if the part has none, else co-caster; a self "streamer" move bumps the current streamer to co-caster. Casting is pending-only, but **removing yourself** also works on decided matches (you shouldn't stay credited for a cast that never happened). The Matches page's **Cast** view lists upcoming parts still needing a caster; its filters are URL-addressable (e.g. `?view=cast`, `?caster=uncasted`).
