...
- **Caster self-service:** any logged-in user can add/move/remove **themselves** on a pending match's part (`…/parts/:part_id/casters/me`, `player.ts`) — streamer if the part has none, else co-caster; a self "streamer" move bumps the current streamer to co-caster. Casting is pending-only, but **removing yourself** also works on decided matches (you shouldn't stay credited for a cast that never happened). The Matches page's **Cast** view lists upcoming parts still needing a caster; the Matches filters are URL-addressable (e.g. `?view=cast`, `?status=scheduled`).
...
Not at the match level. Swiss pairings and the bye recipient are generated by the algorithm and committed when the round is created — matches are never re-paired, and the bye is never reassigned. What an admin **can** do is swap the **occupants** of two same-phase slots (`POST …/slots/swap`): identity moves, the seat (seed, division, history) stays, so the people in two pending matches trade places. The swap refuses (`SLOT_HAS_RESULTS`) once either seat has any decided match — including a bye — so results can never be reattributed; cross-division swaps lock after start. The only influence is *indirect and before Start*: seed order (`/slots/reorder`) and division assignment. What an admin **can** edit afterward are match _results_ (`handleRetroEditMatch` — winner / status), and a match's _map_ and _schedule_ — never who plays whom. A reported result can't be edited once a downstream round has reported matches.
