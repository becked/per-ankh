...
- **A fourth chart shipped: per-player nation picks.** Beyond the three-chart MVP, the Players tab also renders a per-participant civ-portfolio bar (`computePlayerPicks`, `stats.ts`; `playerPicksOption`, `src/lib/tournament/stats-charts.ts`), returned as `player_picks` on the Plane A `/stats` payload. It's an early slice of the deferred **Plane B2** (participant-dimension save-content, §4): it reads `player_summaries` (nation + `is_winner`) attributed through the 0024 match snapshots + `slot_a/b_player_index` → `(game_id, player_index)` — exactly the B2 mapping §6/§8 describe — but it's delivered on the **Plane A** endpoint and computed in `computeCompetitionStats`, *not* through the ChartBundle machinery. So the clean two-plane split (§2) has one deliberate crossing: a save-content chart on the tournament-native endpoint. B2 proper — the §5 UI generalization over the ChartBundle — is still unbuilt; this is a hand-rolled forerunner, not that generalization.
- **One shared `player_summaries` batch loader.** `loadNationsForMatches` (nation-only, for match serialization) and the picks path both need per-`(game_id, player_index)` roster fields, so they share `loadPlayerSummaryFieldsForMatches` (`public.ts`): a chunked (`ceil(N/50)`, under D1's 100-param cap) SELECT of `(nation, is_winner)` keyed `${game_id}:${player_index}`, of which `loadNationsForMatches` is now a nation-only projection. This also chunked `loadNationsForMatches`, which previously used a single unbounded `IN (...)` that would throw once a serialized match set referenced >100 linked games.
- **Plane A `/stats` now touches save content — it no longer scales with matches alone.** The picks chart adds a `loadPlayerSummaryFieldsForMatches` sweep over every linked game on every (uncached) `/stats` request — a games-scaling load the endpoint didn't carry when §6/§8 settled "Plane A uncached because its cost ≈ `/standings` and scales with matches, not games." The Plane A response is now standings (recomputed) + casters + a per-game `player_summaries` read. **Resolved (2026-07-07, code-verified):** the sweep is one narrow `(nation, is_winner)` projection, an order of magnitude under Plane B's cached per-turn bundle, so Plane A stays uncached in v1 — measure before caching. The quantified reasoning and the if-slow plan (whole-blob, key discriminator, standings-stays-embedded, TTL) live in §8.
...
- **Caching — ships in v1 for Plane B; Plane A uncached** (§6). The target tournament's corpus is hundreds of games, putting the bundle compute well past the `/standings` per-read cost that Plane A shares. Key-drift invalidation covers all tournament mutations; the two uncovered paths (game deletion, same-version re-derivation) are bounded by the 24h TTL.
  ...
  **Plane A stays uncached — the nation-picks caveat, quantified, does not flip it** (2026-07-07 review, code-verified). The later picks chart made `/stats` read `player_summaries` per linked game (§7 Build status), so it *does* scale with games — but the load is one narrow sweep, not Plane B's per-turn monster. Measured for a ~300-game tournament, uncached `/stats` is ~10 indexed SELECTs: `computeStandingsResponse` (2 — the accepted `/standings` cost) + the duplicate `loadMatches` + `loadUserIdentitiesForMatches` (2) + `loadPlayerSummaryFieldsForMatches` (`ceil(G/50)` ≈ 6, a *narrow* `(nation, is_winner)` projection, 2 rows/game → ~600 tiny rows). Plane B's cached bundle is five query families × `ceil(N/50)`, including the wide `game_player_turn` per-turn series (tens of thousands of wide rows) — an order of magnitude more. The picks sweep is the *cheapest* games-scaling piece, so it doesn't reach the bar that earned Plane B its cache. **Decision: uncached in v1; measure before caching.** If it ever measures slow:
  - **Cache the whole `/stats` blob, never picks-only.** Picks-only buys back only that one narrow SELECT while still recomputing standings (the larger remaining cost) and the caster leaderboard every request, and it needs the *same* `updated_at`+`parser_version` key plus its own discriminator — no simpler to key, a second invalidation surface for the least of the cost.
  - **Key it with a payload discriminator** so it can't collide with the Plane B bundle under the shared `…:tournament:{id}:{updated_at}` key (`cacheKeyToString`, `cache.ts`): extend the tournament `StatsCacheKey` arm with `plane: "games" | "competition"` and render `…:tournament:{id}:{plane}:{updated_at}` (Plane B's old entries orphan and die by TTL). Keep `parser_version` in the key — `/stats` embeds save content via `player_picks`, so a reparse-all should orphan it, exactly as it does Plane B.
  - **Leave standings embedded — caching `/stats` does not stale it.** Every standings- or caster-affecting mutation bumps `updated_at` (report-via-upload `games.ts:828`, retro-edit `admin.ts:2292`, forfeit-via-withdraw `admin.ts:1476`, caster edits `player.ts:529`, …), so key-drift invalidates the competitive record immediately. The two uncovered gaps leave `winner_slot_id`/`status` untouched (the `0013` game-delete trigger explicitly preserves winner/status), so they can only stale `player_picks` — the same profile Plane B already ships. Splitting standings back out to a live fetch would undo the self-contained `/stats` decision for zero freshness gain.
  - **24h TTL is enough**, same call as Plane B, with a strictly narrower blast radius (the gaps touch only `player_picks`). If a window ever matters, bump `updated_at` at the writer (game-deletion path near `games.ts:828`; single-game re-derive) — that fixes both planes at once; don't add a Plane-A-specific invalidation.
