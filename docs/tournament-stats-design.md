...
- **Plane A `/stats` now touches save content — it no longer scales with matches alone.** The picks chart adds that `loadPlayerSummaryFieldsForMatches` sweep over every completed-match game on every (uncached) `/stats` request — a games-scaling load the endpoint didn't carry when §6/§8 settled "Plane A uncached because its cost ≈ `/standings` and scales with matches, not games." The Plane A response is now standings (recomputed) + casters + a per-game `player_summaries` read, so revisit the Plane-A-caching decision (§6 / §8) with that in mind.
