...
- **Errors:** `401 UNAUTHORIZED`.
- **Notes:** There is no by-user-id form of this route, by design — a player sees only their own list. The response carries no rating, win probability or score, and neither does the table behind it (migration 0046); the model runs entirely inside the Worker. The list is rebuilt by the nightly cron and ordered by when each opponent was last active, so its order is not a ranking. An empty list is a normal answer.
