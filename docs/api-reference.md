...
| `tournament_view` | 2400 reads / hr per IP | the tournament page reads: detail, standings, bracket, rounds, matches, match detail, both stats endpoints, and the profile Tournaments tab — plus the **home page**, whose Featured Tournament panel spends 3 (detail, standings, matches) per load. ~4 reads per page load (6 on stats), so ~600 page loads an hour. The ceiling is the `TOURNAMENT_VIEW_PER_HOUR` var, so it can be retuned mid-event with `wrangler secret put` instead of a redeploy — until the next deploy restores the `wrangler.toml` value |
| `tournament_list_view` | 600 reads / hr per IP | `GET /v1/tournaments` — the `/tournaments` index and the admin dashboard. Its own budget, not `tournament_view`'s: a list read on a busy index must not decide when `/tournaments/[slug]` starts refusing. Own ceiling var (`TOURNAMENT_LIST_VIEW_PER_HOUR`), retuned the same way |
...
| `season_view` | 600 reads / hr per IP | `GET /v1/stats/players`. Its own budget rather than a share of `anon_read`: `/players` and `/games/*` are different populations, and pooling them would let a crawl of game pages decide when the players board starts refusing. Not a share of `global_stats_view` either — `/players` is anonymous where `/stats` is session-gated. One read per `/players` load — the board named in the URL, and only that board — so 600 is 600 page loads an hour. Own ceiling var (`SEASON_VIEW_PER_HOUR`), retuned like the others |
| `home_summary_view` | 600 reads / hr per IP | `GET /v1/home-summary`. Its own budget rather than a share of `anon_read` even though one home page load spends both: `anon_read` at 200/hr is already the tightest ceiling that page meets, so pooling would halve its headroom against itself. Not a share of `global_stats_view` either — home is anonymous where `/stats` is session-gated. One read per home page load, so 600 is 600 loads an hour. Own ceiling var (`HOME_SUMMARY_VIEW_PER_HOUR`) |
...
- **Auth:** Public.
- **Query:** `since` / `until` (optional, `YYYY-MM-DD`, `until` exclusive) — counts only games uploaded in the window (`created_at`, server-authoritative); the `/players` page's season boards, past seasons included. Invalid values are `400 INVALID_QUERY`. A closed window (`until` in the past) is **not** immutable — a visibility toggle, a newly linked online id, the reindex sweep that backfills `player_summaries.online_id`, and a deleted game all move a past window's counters — so every window, closed ones included, answers `max-age=300, s-maxage=60`. The `/players` page also sends `v` — its row-shape version (`PLAYERS_SHAPE_VERSION`, `src/lib/api-cloud.ts`), unread by the Worker: the URL is the cache key, so bumping it orphans the cached boards that predate a new field instead of leaving them a few minutes behind the deploy.
- **Response 200:** `{ players: [{ user_id, display_name, slug, avatar_url, duels_network, duels_cloud, ffas, total }] }`, ordered by total desc. `avatar_url` is the player's Discord avatar, built server-side from the stored hash (`buildAvatarUrl`, Discord's default avatar when they never set one) — the same select-use-don't-serialize shape the profile and standings payloads use. `slug` is the profile slug (null for a user without one), so a row links straight to `/u/<slug>` instead of taking the id permalink's 307 — the same field, for the same reason, that `GET /v1/users/public-search` returns. Playing is counted, not uploading: anyone's upload credits every human seat — the uploader via their claimed seat, others by matching the seat's online id (`player_summaries.online_id`, migration 0044) against `user_online_ids`. Matches double-uploaded by both players count once per player (deduped on `xml_game_id`). **Public games only** (`is_public = 1`, on every credit path) — the same visibility rule the profile card and the global corpus enforce: a save its owner kept private counts for nobody, and a match counts as soon as any one of its uploads is public. Duel = exactly two humans, split by game mode (`NETWORK` / `PLAY_BY_CLOUD`); FFA = 3+ humans; the remainder is derived client-side. Two uploads of one match can disagree on `game_mode` (each client reports the mode it ran in), and the non-local mode wins — a match someone played over the network is a network match however the other seat sat down at it; `NETWORK` over `PLAY_BY_CLOUD` if a match ever reports both.
- **Errors:** `429 RATE_LIMIT_SEASON`.
- **Notes:** Its own `season_view` bucket (600 reads/hr per IP; scraper UAs exempt), not a share of `anon_read`. Exposes only display names, Discord avatars, and counts — never online ids. Cached `public, max-age=300, s-maxage=60`. Backfill for pre-0044 rows is the admin reindex sweep (no re-uploads).
...
The site-admin featured set (see [Site admin: featured videos](#site-admin-featured-videos)), newest video first. The home page puts these at the front of its Recent Videos strip, ahead of the merged creator and tournament feeds — starring a video promotes it rather than giving it a tile of its own; the writes that curate it stay admin-only.
...
- **Query:** `slice` (default `duel`; `all`|`duel`|`ffa`|`single_player`), `nation` (a nation zType, e.g. `NATION_ROME`; default none) and `period` (default `all`; `all`|`12m`|`6m`). All three parse forgivingly like `?scope=` elsewhere — an unknown slice falls back to the default, an unknown nation to no facet and an unknown period to all-time, so a stale bookmark degrades to a neighbouring view instead of `400`ing.
...
- **Notes:** The slices are roster **compositions** — `duel` is exactly two players both human, `ffa` three or more humans, `single_player` exactly one — and they do **not** partition the corpus: a game with two humans and any AI matches none of them, so it appears only under `all`. A `nation` selection narrows twice, the games *and* the seats within them, so a Rome facet over a Rome-vs-Greece duel bands only the Roman's rows. `period` is the third facet and narrows the games only: it filters on `games.save_date` — when the game was **played**, not when it was uploaded — so an old save uploaded last week is old-meta evidence and falls outside a window. It says nothing about which seats are focal, since every seat of a recent game is a recent seat, and a game with no `save_date` is not datable and drops out of every window while still counting under `all`. KV-cached, keyed on `{ slice, nations, period, parser_version }`, with all 56 all-time selections warmed nightly by cron (one pattern per slice); a narrowed window is built on demand and then cached for 24h with serve-stale behind it. The key carries the period **token**, not its resolved cutoff date, so the window's opening edge can trail the clock by up to a cache lifetime — keying on the date would roll the key daily and defeat serve-stale for a distinction "the last six months" is not making. A miss computes in the request and never refuses; across a parser-version bump the previous entry is served stale while the new one rebuilds in the background, but never across a `BUNDLE_SCHEMA_VERSION` bump — that changes the bundle's shape. Edge-cached 60s (`s-maxage`), no browser cache.

### `GET /v1/home-summary`

The bundle fields the home page's stats panels draw, over the unfaceted `duel` slice — a trimmed, public projection of the entry [`GET /v1/stats`](#get-v1stats) serves signed-in visitors by default.

- **Auth:** Public. `/v1/stats` is session-gated on who may spend a whole-corpus aggregation; this endpoint can't spend one. It reads the precomputed KV entry and nothing else — its handler's env type carries no `SHARE_DB`, so it structurally cannot reach `buildChartBundle` — which is what makes an anonymous door onto the same numbers safe.
- **Response 200:** `{ summary: HomeStatsSummary | null }` — `summary` carries `nationWinRate`, `capitalFamilyWinRate` and `startingArchetypeWinRate`, and nothing beside them (not even `meta`: the corpus size and parser version are for a consumer that branches on them, and no home panel does). An envelope rather than a handful of nullable fields: one KV entry answers for all three, so they are present together or not at all. `null` on a cache miss, which the home page renders by dropping the whole stats region rather than showing empty charts.
- **Errors:** `429 RATE_LIMIT_HOME_SUMMARY`.
- **Notes:** Trimmed because the full bundle's bulk is `wonderStats` / `yieldCurves` / `lawTiming`, none of which home reads, and home is the page most likely to be a visitor's first byte of the site. Each field ships whole except `startingArchetypeWinRate`, which is floored at **50 games** server-side (`HOME_ARCHETYPE_MIN_GAMES`) — a correctness filter, not a presentation one: without it the panel shows a 73% win rate off eleven games. So home's archetype panel and `/stats`' show different rows for the same corpus, deliberately and without caveat text; `/stats` is the surface for the whole distribution. The row caps the panels apply (top 7, ranked by games played — which is also the number each row leads with) are the frontend's, so they move without a Worker deploy. Never computes on a miss. What keeps misses rare is the nightly precompute plus the hourly warm (`STATS_WARM_CRON`), which cover exactly the unfaceted slice this reads; across a parser-version bump it serves the previous entry stale, like `/v1/stats`, so a Worker deploy doesn't blank the home page's stats region until the warm runs. Nothing rebuilds behind the stale answer — this handler structurally cannot — so the warm cron is what ends that window. Its own `home_summary_view` bucket (600 reads/hr per IP), one slot per home page load. Edge-cached 60s (`s-maxage`), no browser cache.
...
- **Query:** `round_id`, `phase`, `division`, `slot_id`, `status` (all optional, in-memory filters; an unknown value filters to nothing rather than erroring). `?status=pending` is the schedule read — the home page's Featured Tournament panel uses it so a landing page carries what is still to come rather than an event's whole record.
