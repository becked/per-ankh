...
**Design system.** The app has one, in code rather than prose: the `--color-surface-{deep,sunken,raised}` ladder in `src/app.css`, whose comments say what each level is for, and `Panel` / `PanelInset` (`$lib/ui`) for the titled-panel-over-raised-subpanel shape the home page is built from. Match the surfaces around you before inventing a layout — an `<h2>` in game detail is `text-lg`, while `Panel`'s own title is `text-base`; a disclosure is the chevron-with-`rotate-90` idiom; a page scrolls with `cloud-scroll` + `autohideScroll`; a profile link goes through `ProfileLink`; a two-way switch is the lit-thumb segmented control shared by `TournamentViewTabs.svelte` and the `/players` board switch. Reserve the slot for an icon that may not resolve and the height for a row count that varies, so a table stays aligned and a panel doesn't resize under the pointer. Prefer Old World's own glyph to a hand-rolled SVG or an emoji.

**Copy.** A heading names what is on screen, not the machinery behind it — "Turn summary", not "Logged this window". Use the game's own string where it has one: the baked tables in `src/lib/generated/` (`name-text.ts`, `tech-names.ts`, and the other `*-names` modules) carry the display names that differ from `formatEnum()`, which handles the rest. Don't caption an interaction the UI already affords. A tooltip claims only what the data supports. A confirm dialog for something irreversible says so.
