<script lang="ts">
	...
	const filteredSortedCities = $derived(() => {
		...
		const column = CITY_COLUMNS.find(
			(col) => col.key === tables.cities.sortColumn,
		);
		if (column) {
	...
	const improvementPivotData = $derived(() => {
		if (!improvementData || improvementData.improvements.length === 0)
			return [];
	...
	const scienceChartOption = $derived(
		createYieldChartOption(
			"YIELD_SCIENCE",
			"Science Production",
			"Science per Turn",
			chartFilters.science,
		),
	);
	const civicsChartOption = $derived(
		createYieldChartOption(
			"YIELD_CIVICS",
			"Civics Production",
			"Civics per Turn",
	...
	const trainingChartOption = $derived(
		createYieldChartOption(
			"YIELD_TRAINING",
			"Training Production",
			"Training per Turn",
			chartFilters.training,
		),
	);
	const growthChartOption = $derived(
		createYieldChartOption(
			"YIELD_GROWTH",
			"Growth Production",
			"Growth per Turn",
	...
	const cultureChartOption = $derived(
		createYieldChartOption(
			"YIELD_CULTURE",
			"Culture Production",
			"Culture per Turn",
			chartFilters.culture,
		),
	);
	const happinessChartOption = $derived(
		createYieldChartOption(
			"YIELD_HAPPINESS",
			"Happiness Production",
			"Happiness per Turn",
			chartFilters.happiness,
		),
	);
	const ordersChartOption = $derived(
		createYieldChartOption(
			"YIELD_ORDERS",
			"Orders",
			"Orders per Turn",
	...
	const moneyChartOption = $derived(
		createYieldChartOption(
			"YIELD_MONEY",
			"Money Income",
			"Gold per Turn",
	...
	const discontentChartOption = $derived(
		createYieldChartOption(
			"YIELD_DISCONTENT",
			"Discontent",
			"Discontent per Turn",
			chartFilters.discontent,
		),
	);
	const ironChartOption = $derived(
		createYieldChartOption(
			"YIELD_IRON",
			"Iron Production",
			"Iron per Turn",
	...
	const stoneChartOption = $derived(
		createYieldChartOption(
			"YIELD_STONE",
			"Stone Production",
			"Stone per Turn",
	...
	const maintenanceChartOption = $derived(
		createYieldChartOption(
			"YIELD_MAINTENANCE",
			"Maintenance Costs",
			"Maintenance per Turn",
			chartFilters.maintenance,
		),
	);
		...
		<Tabs.Root bind:value={activeTab}>
			...
			>
				...
				{#if processedEventLogs === null}
					<p class="p-8 text-center italic text-brown">Loading event logs...</p>
										...
										<span class="inline-flex items-center gap-1">
											...
											{#if tables.events.sortColumn === "turn"}
												<span class="text-orange"
													>{tables.events.sortDirection === "asc"
														? "↑"
														: "↓"}</span
												>
											{/if}
			...
			>
				...
				{#if allYields === null}
					<p class="p-8 text-center italic text-brown">Loading yield data...</p>
				...
				{:else}
					...
					{#if civicsChartOption}
						<ChartContainer
							option={civicsChartOption}
							height="400px"
							title="Civics Production"
						/>
					{/if}
					...
					{#if happinessChartOption}
						<ChartContainer
							option={happinessChartOption}
							height="400px"
							title="Happiness Production"
						/>
					{/if}
			...
			>
				...
				{:else if unitsProduced === null}
					<p class="p-8 text-center italic text-brown">Loading unit data...</p>
				{:else}
					<p class="p-8 text-center italic text-brown">
			...
			>
				...
				{:else}
					...
					<div class="mb-4 flex flex-wrap items-end gap-3">
						...
						<Select.Root
							type="multiple"
							bind:value={tables.improvements.filters}
						>
							<Select.Trigger
								class="relative flex w-32 items-center justify-between rounded border-2 border-black py-2 pl-9 pr-8 text-sm text-tan"
								style="background-color: #201a13;"
								...
								<tr>
									...
									{#each displayedImprovementNations as nation (nation)}
										<th
											class="hover:bg-brown/20 cursor-pointer select-none whitespace-nowrap border-b-2 border-brown p-3 text-center font-bold text-brown"
											onclick={() =>
												toggleSort("improvements", `nation:${nation}`)}
										>
											<span
												class="inline-flex items-center justify-center gap-1"
											>
