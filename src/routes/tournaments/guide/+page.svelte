...
<main class="cloud-scroll flex-1 overflow-y-auto px-4 py-8" use:autohideScroll>
	...
	<div class="mx-auto grid max-w-screen-2xl gap-x-[4.5rem] gap-y-6 md:grid-cols-2">
					...
					<div class="space-y-3 text-xs text-tan">
						<section>
							...
							<p class="opacity-80">
								...
								single-elimination
								<strong>championship bracket</strong> to crown the champion.
								Here's the whole path from sign-up to champion.
							</p>
						</section>
						...
						<section>
							...
							<p>
								...
								two divisions. Divisions exist only so rounds can run in parallel
								and finish faster — they carry no competitive weight once Swiss
								ends. You can withdraw any time before the tournament starts.
							</p>
						</section>
						...
						<section>
							...
							<p>
								...
								Round 1 is paired by seed; later rounds pair players with similar
								records and avoid rematches. Report a result by uploading the
								finished save for the match — the winner is read from the game, no
								opponent confirmation needed.
							</p>
						</section>
						...
						<section>
							...
							<p>
								You keep playing rounds until you hit one of two thresholds: reach
								the
								<strong>win threshold</strong> and you've <em>advanced</em> —
								you're in the championship bracket and sit out the rest of Swiss;
								reach the <strong>loss threshold</strong> and you're
						...
						<section>
							...
							<p>
								...
								— there's no cap and no per-division cutoff. All qualifiers from
								both divisions are ranked into a single combined list (the seeding
								cascade) and seeded into one single-elimination bracket using
								standard 1-vs-N order (seed 1 plays the lowest-ranked qualifier,
								etc.). If the qualifier count isn't a power of 2, the top seeds get
								round-1 byes. Win through to the Grand Finals to take the title.
							</p>
						</section>
					...
					<div class="space-y-3 text-xs text-tan">
						<section>
							...
							<p class="mb-2 opacity-80">
								...
								tied. The tiebreakers only set bracket <em>seeding</em> — never
								who's in or out.
							</p>
							<ol class="ml-4 list-decimal space-y-1">
								...
								<li>
									<strong>Initial seed</strong> — if players are still tied after all
									of the above, the higher initial Swiss seed seeds higher. This always
									settles the order, so seeding never needs a manual tiebreak.
								</li>
							</ol>
						</section>
					</div>
				...
				<div class="rounded-lg p-3" style="background-color: #35302B;">
					<div class="space-y-3 text-xs text-tan">
						<section>
							...
							<p>
								Every match is auto-assigned a map from the tournament's map pool
								when its round is generated — nobody picks. The assignment follows
								a priority order:
							</p>
							<ol class="ml-4 mt-2 list-decimal space-y-1">
								<li>
									<strong>A fresh base map.</strong> As long as the pool still has
									a base map script that <em>neither</em> you nor your opponent has
									played earlier in the tournament, you'll be given one of those.
								</li>
								<li>
									...
									(size, options) you haven't actually seen, so the map at least
									plays differently.
								</li>
							</ol>
							<p class="mt-2 opacity-80">
								...
								depends on the pool having enough distinct base scripts: with only
								a couple of base maps, later rounds will have to repeat.
							</p>
						</section>
					</div>
				</div>
