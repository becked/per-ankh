<script lang="ts">
	...
	// View control. The view is deep-linkable via ?view= (defaults stay out of the
	// URL; state changes replace, not push, history). The active clock comes from
	// the layout's shared context — the header toggle owns it and persists the
	// choice — and is reflected into ?zone= below, so a link can also point at a
	// given clock.
	...
	$effect(() => {
		...
		if (clock.zone !== "local") parts.push(`zone=${clock.zone}`);
	...
	const eventsByDay = $derived.by(() => {
		...
		for (const np of partition.scheduled) {
			const key = scheduledDayKey(np.part.scheduled_at, clock.zone);
			if (!key) continue;
			(byDay[key] ??= []).push(np);
		}
		return byDay;
						...
						{#each calendarCells as cell, i (i)}
							...
							{:else}
								...
								<div
									...
									style="background-color: rgb(var(--color-surface-raised));"
								>
									<span class="text-[11px] font-bold text-tan opacity-70">
										{cell.day}
									</span>
