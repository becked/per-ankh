...
>
	{#each series as s}
		...
		<ToggleGroup.Item
			value={s.name}
			class="cursor-pointer rounded border-2 px-3 py-1 text-xs transition-all duration-200"
			style="background-color: {isSelected
				? s.color
				: 'transparent'}; border-color: {s.color}; color: {isSelected
				? getContrastColor(s.color)
				: s.color}; opacity: {isSelected ? 1 : 0.5};"
		>
			{s.name}
		</ToggleGroup.Item>
	{/each}
