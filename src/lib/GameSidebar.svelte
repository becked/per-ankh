<script lang="ts">
	...
	// Context menu state
	let contextMenu = $state<{ x: number; y: number; game: GameInfo } | null>(
		null,
	);
	...
	async function moveToCollection(collectionId: number) {
		...
		try {
			await api.moveMatchesToCollection(
				[contextMenu.game.match_id],
				collectionId,
			);
			refreshData.trigger();
		} catch (err) {
	...
	const filteredGames = $derived(
		games.filter((game) => {
			...
			const nation = game.save_owner_nation
				? formatEnum(game.save_owner_nation, "NATION_").toLowerCase()
				: "";
...
>
	...
	<div class="sidebar-content flex-1 overflow-y-auto px-2 pb-2 pt-2">
		...
		{:else if error}
			<div class="p-4 text-center font-bold text-orange">Error: {error}</div>
						...
						>
							...
							{#if game.save_owner_nation}
								<span class="nation-badge"
									>{formatEnum(game.save_owner_nation, "NATION_")}</span
								>
							{/if}
						</button>
					{/each}
