<script lang="ts">
	...
	let { tournament, combined, busy = false, onConfirm }: Props = $props();
...
<Popover
	bind:open
	ariaLabel="Transition to championship"
	contentClass="w-[min(95vw,48rem)]"
>
	...
	<header class="mb-3 flex items-start justify-between gap-3">
		<h2 class="text-lg font-bold text-tan">Transition to championship</h2>
		<button
			type="button"
	...
	<div class="space-y-3 text-xs text-tan">
		{#if needsOverride}
			<div class="rounded border border-red-500 bg-red-500/10 p-2 text-red-300">
		...
		<section>
			<header class="mb-2 flex items-baseline justify-between">
				...
				<span class="opacity-70">
					{previewSeeds.length} qualifier{previewSeeds.length === 1 ? "" : "s"}
		...
		<section>
			...
			{#if manualOpen}
				<p class="mb-2 opacity-70">
					Tick the slots that should be in the bracket. Use ↑ ↓ to reorder; the
					order top-to-bottom becomes seed 1, 2, 3, …
				</p>
				<table class="w-full">
					<thead>
			...
			{:else}
				<p class="opacity-70">
					Auto-promote: every player at status "advanced" enters the bracket in
					seeding-cascade order.
				</p>
			{/if}
		</section>
	</div>
