...
describe("tournament flow", () => {
	it("runs a 4-vs-4 swiss through a 1-round configuration, then completes the championship", async () => {
		...
		// → 4 championship slots → semifinal + final.
		...
		// Act — generate + start round 1 for both divisions, report results
		// with slot_a always winning.
		...
		// Start it, then report both matches.
		...
		// Assert — terminal status reached, every match has a winner.
	...
	it("handles a 3-vs-3 swiss tournament with byes in round 1", async () => {
		...
		const byesA = matches.filter(
			(m) => m.status === "bye" && t.slotsByDivision.A.some((s) => s.slotId === m.slot_a_id),
		);
		const byesB = matches.filter(
			(m) => m.status === "bye" && t.slotsByDivision.B.some((s) => s.slotId === m.slot_a_id),
		);
