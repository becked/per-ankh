...
//
// The four corpus reads resolve corpus → check cache → compute on miss →
// return the payload. Each corpus carries two payloads built in one pass and
// cached under two keys (stats/cache.ts). The records are their own endpoint
// because they are ~60-70 KB gzipped on a 154 KB bundle and only the Records
// tab reads them, so a surface pays for them when that tab opens rather than
// on every load.
//
// `/v1/stats/players` is not a bundle at all — it counts games played per user
// straight out of D1, uncached — and shares this file for the corpus it reads
// rather than for the shape it returns. `/v1/home-summary` is a trimmed,
// public projection of `/v1/stats`'s cached entry that never computes one.
...
import type { StatsCacheEnv, StatsPayload } from "./cache";
